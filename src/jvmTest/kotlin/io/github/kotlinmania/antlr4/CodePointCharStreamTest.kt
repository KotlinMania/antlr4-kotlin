...
class CodePointCharStreamTest {
    ...
    fun singleEmojiCodePointLookAheadShouldReturnCodePoint() {
        val stream = CharStreams.fromString(emoji(0x1F4A9))
    ...
    fun textWithMixedCodePointWidths() {
        val stream =
            CharStreams.fromString(
                "01234" + emoji(0x1F522) + "6789",
            )

        assertEquals("34" + emoji(0x1F522) + "67", stream.getText(Interval.of(3, 7)))
    }
    ...
    fun smpContentsShouldUse32BitBuffer() {
        val stream = CharStreams.fromString("hello " + emoji(0x1F30D))
